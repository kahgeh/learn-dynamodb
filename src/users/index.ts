...
exports.handler = async (event, context): Promise<{ statusCode: number, body: any }> => {
    ...
    switch (routeKey) {
        ...
        case (RouteKeys.GET_USER): {
            ...
            const result = await docClient.get({ TableName: 'Users', Key: { id: userId } })
                .promise();
        ...
        case (RouteKeys.PUT_USER): {
            ...
            await docClient.put({ TableName: 'Users', Item: { id: userId, ...user } })
                .promise();
        ...
        case (RouteKeys.DELETE_USER): {
            var docClient = new AWS.DynamoDB.DocumentClient();
            const userId = event.pathParameters.userId;
            await docClient.delete({ TableName: 'Users', Key: { id: userId } })
                .promise();
